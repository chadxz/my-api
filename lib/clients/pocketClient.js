...
PocketClient.prototype.getArticles = function (params, callback) {
  if (!_.get(params, 'access_token')) {
    callback(new errors.RequiredParamMissingError('params.access_token', 'string'));
    return;
  }

  var whitelistedParams = ['access_token', 'state', 'favorite', 'tag', 'contentType',
    'sort', 'detailType', 'search', 'domain', 'since', 'count', 'offset'];
  var options = {
    uri: url.resolve(apiBase, 'get'),
    json: _.assign({
      consumer_key: this.consumerKey
    }, _.pick(params, whitelistedParams))
  };
  ...
  request.post(options, function (err, res, body) {
    ...
    if (!tools.isOk(res.statusCode) || typeof body !== 'object' || typeof body.list !== 'object') {
      callback(new errors.UnexpectedServerResponseError(res));
      return;
    }
     ...
     */
    var articles = Object.keys(body.list).map(function (key) {
      return body.list[key];
    });

    /*
     * Object.keys does not guarantee any specific order, which messes up
     * the ordering, so we have to re-order the list by timestamp.
     */
    var sortField;
    var sortOrder;
    ...
    if (!params.sort || params.sort === 'newest') {
      ...
      sortField = 'resolved_url';
      sortOrder = 'asc';
    }

    var result = _.sortByOrder(articles, [sortField], [sortOrder]);
