...
class PocketClient {
  ...
  constructor(consumerKey) {
    ...
    this.apiBase = "https://getpocket.com/v3/";
  }
   ...
   * @returns {Promise<String>} Resolves with the request token, or rejects with
   *  the error.
   */
  async getRequestToken(redirectUri) {
    if (!redirectUri) {
      throw new RequiredParamMissingError("redirectUri", "string");
    }

    const options = {
      uri: resolve(this.apiBase, "oauth/request"),
      json: {
   ...
   * @returns {Promise<Object>} Resolves with then authorization, or rejects with
   *  the error. The authorization object has structure { access_token, username }
   */
  async getAuthorization(requestToken) {
    if (!requestToken) {
      throw new RequiredParamMissingError("requestToken", "string");
    }

    const options = {
      uri: resolve(this.apiBase, "oauth/authorize"),
      json: {
        consumer_key: this.consumerKey,
        code: requestToken
      }
   ...
   *  `offset` position
   * @returns {Promise<Array>} Resolves with the pocket articles, or rejects
   *  with the error.
   */
  async getArticles(params = {}) {
    if (!get(params, "access_token")) {
      throw new RequiredParamMissingError("params.access_token", "string");
    }
    ...
    const response = await request.post(options);
    const articlesList = get(response.body, "list");

    if (typeof articlesList !== "object") {
      throw new UnexpectedServerResponseError(response);
    }
