...
PocketWorker.prototype._doWork = function (callback) {
  ...
  async.auto({
    ...
    getPocketArticles: ['getAuthorization', function (done, results) {
    ...
    saveArticles: ['getPocketArticles', function (done, results) {
      // remove any articles tagged 'private'
      var articles = results.getPocketArticles.filter(function (article) {
        return !article.tags || Object.keys(article.tags).every(function (key) {
            return article.tags[key].tag !== 'private';
          });
      });

      articles = _.sortByOrder(articles, ['time_read'], ['desc']);
