...
const Worker = require("./baseWorker");
const vars = require("../vars/pinboardVars");
const errors = require("../errors");
const redisKeys = vars.redisKeys;
const rateLimitsMS = vars.rateLimitsMS;
...
PinboardWorker.prototype.name = "PinboardWorker";
...
PinboardWorker.prototype._doWork = function(callback) {
  ...
  self.redis.get(redisKeys.lastUpdated, function(err, lastLocalUpdate) {
    ...
    self.client.getLastUpdateDate(function(err, pinboardLastUpdateDate) {
      ...
      const msSinceLastUpdated =
        new Date().getTime() - lastLocalUpdate.getTime();
      if (
        pinboardLastUpdateDate > lastLocalUpdate &&
        msSinceLastUpdated >= rateLimitsMS.postsAll
      ) {
      ...
      const throttleTimeRemaining = tools.toSeconds(
        rateLimitsMS.postsAll - msSinceLastUpdated
      );
      callback(
        null,
        "pinboard update throttled. time remaining until next update: " +
