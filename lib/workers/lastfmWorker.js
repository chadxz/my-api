...
const vars = require("../vars/lastfmVars");
...
LastfmWorker.prototype._doWork = function(callback) {
  ...
  self.redis.get(redisKeys.lastUpdated, function(err, lastLocalUpdate) {
    ...
    const msSinceLastUpdated = new Date().getTime() - lastLocalUpdate.getTime();
    ...
    const throttleTimeRemaining = tools.toSeconds(
      rateLimitsMS.defaultLimit - msSinceLastUpdated
    );
    callback(
      null,
      "Last.fm update throttled. Time remaining until next update: " +
