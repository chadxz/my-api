...
router.get('/authorize/callback', isAuthenticated, function (req, res) {
  req.services.pocket.getAccessToken(req.session.requestToken, function (err, authorization) {
    ...
    if (!authorization) {
      res.serverError(new Error('Unable to retrieve access code'));
      return;
    }

    req.services.pocket.setAuthorization(authorization, function (err) {
      if (err) {
        res.serverError(err);
        return;
      }
...
router.get('/unlink', isAuthenticated, function (req, res) {
  req.services.pocket.removeAuthorization(function (err) {
    if (err) {
      res.serverError(err);
      return;
    }
