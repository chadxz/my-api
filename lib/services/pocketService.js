"use strict";
const pocketVars = require("../vars/pocketVars");
...
PocketService.prototype.getLocalAuthorization = function(callback) {
  this.redis.hgetall(pocketVars.redisKeys.authorization, function(
    err,
    authorization
  ) {
    if (err) {
 ...
 * @memberof PocketService
 */
PocketService.prototype.setLocalAuthorization = function(
  authorization,
  callback
) {
  this.redis.hmset(pocketVars.redisKeys.authorization, authorization, function(
    err
  ) {
    if (err) {
      callback(err);
...
PocketService.prototype.removeLocalAuthorization = function(callback) {
  this.redis.del(pocketVars.redisKeys.authorization, function(
    err /*, affectedCount */
  ) {
    if (err) {
      callback(err);
...
PocketService.prototype.removeLocalArticles = function(callback) {
  this.redis.del(pocketVars.redisKeys.articles, function(
    err /*, affectedCount */
  ) {
    if (err) {
      callback(err);
...
PocketService.prototype.getLocalArticles = function(callback) {
  this.redis.get(pocketVars.redisKeys.articles, function(err, articlesJSON) {
    if (err) {
      callback(err);
      return;
    }

    callback(null, tools.parseJSON(articlesJSON));
  });
};

PocketService.prototype.startWorker = function() {
  this.worker.start(pocketVars.rateLimitsMS.userLimit * 4);
};
